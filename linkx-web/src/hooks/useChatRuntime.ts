...
import { splitChatTextSegments } from '../utils/chatText'
...
export function useChatRuntime(options: UseChatRuntimeOptions) {  // 行注：导出当前能力
  ...
  function getMessageTextSegments(content: string) {
    return splitChatTextSegments(content)
  }

  function syncTrayUnreadCount() {
    const api = getElectronAPI()
    if (!api?.setTrayUnreadCount) {
      return
    }
    const total = options.sessions.value.reduce((sum, session) => sum + Math.max(0, Number(session.unreadCount) || 0), 0)
    void api.setTrayUnreadCount(total)
  }

  watch(
    () => options.sessions.value.map(session => `${session.targetId}:${session.sessionType}:${session.unreadCount ?? 0}`).join('|'),
    () => {
      syncTrayUnreadCount()
    },
    { immediate: true }
  ...
  let disconnectChatSocketRef: () => void = () => undefined
  ...
  const realtimeService = createChatRealtimeService({  // 行注：开始解构当前返回值
    ...
    onForceLogout: payload => {
      options.message.warning(payload?.reason || '您的账号已在其他设备登录或已被管理员强制下线')
  ...
  disconnectChatSocketRef = disconnectChatSocket
